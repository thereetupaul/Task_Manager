...
def get_posts(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user),limit: int = 10,skip: int =0,search: Optional[str]=""):
    ...
    print(search)
    print(limit)
    #posts = db.query(models.Post).filter(models.Post.title.contains(search)).limit(limit).offset(skip).all()     #.filter(models.Post.owner_id==current_user.id)
...
def create_posts(post:schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    conn.commit()
    '''
    #not gonna hardcode owner_id -> instead get it from the token 
    #print(current_user.id)

    #new_post = models.Post(title = post.title, content=post.content, published=post.published)
    #print(post.dict())
    new_post = models.Post(owner_id = current_user.id, **post.dict())  #unpacking the dictionary using **
    db.add(new_post)
    db.commit()
    db.refresh(new_post)    #retrieve the new post from database
...
def get_post(id:int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    if not post:
        ...
        #return Response(status_code=status.HTTP_404_NOT_FOUND)       #no return body
